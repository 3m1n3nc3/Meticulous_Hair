...
<section class="page-section color">
    <div class="container box_shadow">
        ...
        <h3 class="block-title alt">
            ...
            <?php echo translate('delivery_options');?>
        </h3>
        <div action="#" class="form-delivery delivery_options">
        </div>
...
<script>
    ...
    function load_orders() {
		var top = Number(200);
		$('.orders').html('<div style="text-align:center;width:100%;height:'+(top*2)+'px; position:relative;top:'+top+'px;"><i class="fa fa-refresh fa-spin fa-3x fa-fw"></i></div>');
        $('.orders').load('<?php echo base_url(); ?>meticulous/cart_checkout/orders');
    }

    function load_delivery_options() {

        var top = Number(200);
        $('.delivery_options').html('<div style="text-align:center;width:100%;height:'+(top*2)+'px; position:relative;top:'+top+'px;"><i class="fa fa-refresh fa-spin fa-3x fa-fw"></i></div>');

        $('.delivery_options').load('<?php echo base_url(); ?>meticulous/cart_checkout/delivery_options',
            function(){
                var top_off = $('.header').height(); 
                $('html, body').animate({
                    scrollTop: $(".delivery_options").offset().top-(2*top_off)
                }, 1000);
            }
        );
    }
    ...
    function load_address_form(){
        ...
        $('.delivery_address').load('<?php echo base_url(); ?>meticulous/cart_checkout/delivery_address',
            function(){
                var top_off = $('.header').height();
    ...
    function load_payments(){
        ...
        if(okay == 'yes'){
            ...
            $('.payments-options').load('<?php echo base_url(); ?>meticulous/cart_checkout/payments_options',
                function(){
                    var top_off = $('.header').height();
                    $('html, body').animate({
    ...
    function delivery_options_check(id){
